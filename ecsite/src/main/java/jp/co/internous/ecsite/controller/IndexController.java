...
//「RequestMapping」アノテーションより localhost:8080/ecsite/でアクセスできるように設定
public class IndexController {
	
	@Autowired
	private UserRepository userRepos;
	//UserエンティティからuserテーブルにアクセスするDAO(Repository)。
	
	@Autowired
	private GoodsRepository goodsRepos;
	//GoodsエンティティからgoodsテーブルにアクセスするDAO(Repository)。
	...
	public String Index(Model m) {
		//トップページ(index.html)に遷移するメソッドです。
		//goodsテーブルから取得した商品エンティティの一覧を、フロントに渡すModelを追加している。
		List<Goods> goods = goodsRepos.findAll();
		m.addAttribute("goods", goods);
		//IndexにgoodsReposで検索した引数のデータを渡す
